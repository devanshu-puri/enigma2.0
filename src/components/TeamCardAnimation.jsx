...
const TeamCardAnimation = ({ cards }) => {
  ...
  useGSAP(() => {
    ...
    // Timeline for fading/scaling between cards
    ...
    for (let i = 0; i < cardsEls.length - 1; i++) {
      tl.to(cardsEls[i], { scale: 0.7, y: -100, opacity: 0, duration: 1, ease: "power1.inOut" }, i);
      tl.to(cardsEls[i + 1], { scale: 1, y: 0, opacity: 1, duration: 1, ease: "power1.inOut" }, i);
    }
  }, { scope: container });
        ...
        <div className="sticky-cards-container relative h-screen w-full">
          {cards.map((card, index) => (
            <div
              ...
              style={{ zIndex: cards.length - index, opacity: index === 0 ? 1 : 0, transform: "scale(1)" }}
            >
              <div className="sticky-card-info bg-black/60 text-white p-4 rounded-xl mb-4 text-center max-w-lg">
                ...
                {card.link && card.link !== "#" && (
                  <a href={card.link} target="_blank" rel="noopener noreferrer" className="linkedin-btn inline-block mt-2">
              ...
              <img src={`assets/team/${card.img}`} alt={card.name} className="sticky-card-image max-h-[70vh] rounded-2xl shadow-lg object-cover" />
            </div>
          ))}
        </div>
      </div>
